...
When changes happen to `FlutterMap`'s internal state (such as a change to the current [`MapCamera`](get-camera-once.md)) it emits a `MapEvent`, which can be handled by you.
